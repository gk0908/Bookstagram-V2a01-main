...
app.post('/upload-files', async (req, res) => {
  const { title, author, description, genre, rating, coverImage, pdfData, fileName, fileSize, userId, featured } = req.body;
  ...
  try {
    ...
    await book.save();
    res.json({ status: 'ok', id: book._id });
  } catch (error) {
    console.error('Error saving book:', error);
    res.status(500).send('Error saving book');
  }
...
app.get('/api/library/featured', async (req, res) => {
  try {
    const limit = parseInt(req.query.limit) || 6;
    const books = await Book.find({ featured: true }).limit(limit).sort({ createdAt: -1 });
    res.json(books);
  } catch (error) {
    console.error('Error fetching featured books:', error);
    res.status(500).send('Error fetching featured books');
  }
});
